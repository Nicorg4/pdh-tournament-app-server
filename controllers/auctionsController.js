...
const purchasePlayer = (req, res) => {
  ...
  const userHasEnoughMoneyQuery = `
    SELECT * FROM users WHERE id = (SELECT owner_id FROM teams WHERE id = ?) AND money >= ?;
    `;
  ...
  connection.beginTransaction((err) => {
    ...
    connection.query(getAuctionQuery, [auctionId], (error, results) => {
      ...
      connection.query(userHasEnoughMoneyQuery, [toTeamId, auctionPrice], (error, results) => {
        if (error) {
          return connection.rollback(() => {
            console.error("Error while checking user has enough money", error);
